...
#if I2S_ESP32_IS_DIR_EN(rx)
	if (dir == I2S_DIR_RX) {
		/* Flush any residual data left in the RX FIFO by the brief
		 * gap between descriptors. Without this, the FIFO holds a
		 * partial sample at the channel boundary, which causes a
		 * half-pair drift (L/R swap) after the GDMA channel reload.
		 */
