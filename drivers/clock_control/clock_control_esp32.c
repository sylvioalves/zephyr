...
#if defined(CONFIG_SOC_ESP32) || defined(CONFIG_SOC_ESP32_NET) || defined(CONFIG_SOC_ESP32S3) || defined(CONFIG_SOC_ESP32S3_NET)
...
#if defined(CONFIG_SOC_ESP32S3_NET)
static void esp32_clock_perip_init(void)
{
	// uint32_t common_perip_clk, hwcrypto_perip_clk, wifi_bt_sdio_clk = 0;
	// uint32_t common_perip_clk1 = 0;

	// soc_reset_reason_t rst_reason = esp_rom_get_reset_reason(1);

	// /* For reason that only reset CPU, do not disable the clocks
	//  * that have been enabled before reset.
	//  */
	// if (rst_reason == RESET_REASON_CPU1_MWDT0 || rst_reason == RESET_REASON_CPU1_SW ||
	// 	rst_reason == RESET_REASON_CPU1_RTC_WDT || rst_reason == RESET_REASON_CPU1_MWDT1) {
	// 	common_perip_clk = ~READ_PERI_REG(SYSTEM_PERIP_CLK_EN0_REG);
	// 	hwcrypto_perip_clk = ~READ_PERI_REG(SYSTEM_PERIP_CLK_EN1_REG);
	// 	wifi_bt_sdio_clk = ~READ_PERI_REG(SYSTEM_WIFI_CLK_EN_REG);
	// } else {
