...
/* Self-delete: a worker aborts itself via its own handle. k_thread_abort on the
 * current thread does not return, so the code after the self-delete call must
 * never run, and the system must stay healthy afterwards.
 */
static volatile bool self_del_before;
static volatile bool self_del_after;
static osal_sem_t self_del_done;

static void self_deleter(void *arg)
{
	ARG_UNUSED(arg);
	self_del_before = true;
	osal_sem_give(self_del_done);
	osal_thread_delete(osal_thread_current());
	/* Unreachable: the abort above does not return. */
	self_del_after = true;
}

ZTEST(osal, test_thread_self_delete)
{
	osal_thread_t t;

	self_del_before = false;
	self_del_after = false;
	self_del_done = osal_sem_create(1, 0);

	t = osal_thread_create(self_deleter, "selfdel", NULL, STACK_SIZE, PRIO);
	zassert_not_null(t);

	/* Wait until the worker is about to self-delete, then give it time to
	 * run past (or not past) the abort.
	 */
	zassert_equal(osal_sem_take(self_del_done, OSAL_FOREVER), OSAL_OK);
	osal_delay_ms(20);

	zassert_true(self_del_before, "worker never ran");
	zassert_false(self_del_after, "code ran after self-delete abort");

	/* System still healthy: a fresh thread runs to completion. */
	worker_ran = 0;
	t = osal_thread_create(prio_worker, "after", (void *)(uintptr_t)9,
			       STACK_SIZE, PRIO);
	zassert_not_null(t);
	osal_delay_ms(20);
	zassert_equal(worker_ran, 9);
	osal_thread_delete(t);

	osal_sem_delete(self_del_done);
}
...
ZTEST(osal, test_busy_wait)
{
	...
	/* A short busy-wait must return without faulting. */
	osal_busy_wait_us(20000);

	/* On real hardware the tick advances during the spin, so uptime must
	 * move forward. native_sim does not advance the simulated clock while
	 * busy-waiting, so the elapsed-time check is skipped there.
	 */
	if (!IS_ENABLED(CONFIG_ARCH_POSIX)) {
		zassert_true(osal_uptime_ms() - t0 >= 18,
			     "busy-wait did not advance uptime");
	}
}
...
/* A zero period is rejected: it would otherwise program k_timer_start with a
 * K_NO_WAIT duration and period, firing once immediately and never repeating
 * (silently degrading a periodic timer to a one-shot).
 */
ZTEST(osal, test_timer_zero_period)
{
	osal_timer_t one = osal_timer_create(on_timer, NULL, false);
	osal_timer_t per = osal_timer_create(on_timer, NULL, true);

	zassert_not_null(one);
	zassert_not_null(per);
	zassert_equal(osal_timer_start(one, 0), OSAL_INVAL);
	zassert_equal(osal_timer_start(per, 0), OSAL_INVAL);
	zassert_equal(osal_timer_start(NULL, 10), OSAL_INVAL);
	osal_timer_delete(one);
	osal_timer_delete(per);
}

/* -------------------------------------------------------------------------
 * Event group: atomic clear-on-wait
 * -------------------------------------------------------------------------
 * A setter posts a matched bit again immediately after the waiter wakes with
 * clear=true. The clear must be atomic with the wait so the re-posted bit is
 * not wiped: a fresh wait must still observe it.
 */
static osal_event_t atomic_evt;

static void evt_reposter(void *arg)
{
	ARG_UNUSED(arg);
	/* First post wakes the waiter. */
	osal_event_set(atomic_evt, EVT_A);
	/* Hammer the same bit so a re-post is likely to land in the window
	 * between the waiter's wake and any (non-atomic) clear.
	 */
	for (int i = 0; i < 200; i++) {
		osal_event_set(atomic_evt, EVT_A);
	}
...
ZTEST(osal, test_event_atomic_clear)
{
	osal_thread_t t;
	uint32_t matched;

	atomic_evt = osal_event_create();
	zassert_not_null(atomic_evt);

	t = osal_thread_create(evt_reposter, "repost", NULL, STACK_SIZE, PRIO);
	zassert_not_null(t);

	matched = osal_event_wait(atomic_evt, EVT_A, false, true, OSAL_FOREVER);
	zassert_equal(matched & EVT_A, EVT_A);

	/* Let the reposter finish hammering the bit. */
	osal_delay_ms(20);

	/* The bit was re-posted after our clearing wait; an atomic clear leaves
	 * those re-posts intact, so it must still be observable.
	 */
	zassert_equal(osal_event_wait(atomic_evt, EVT_A, false, false, OSAL_NO_WAIT) & EVT_A,
		      EVT_A, "re-posted bit was lost by a non-atomic clear");

	osal_thread_delete(t);
	osal_event_delete(atomic_evt);
}

/* wait_all with only some of the requested bits set must not be satisfied. */
ZTEST(osal, test_event_wait_all_partial)
{
	osal_event_t e = osal_event_create();

	zassert_not_null(e);
	osal_event_set(e, EVT_A); /* only one of the two requested bits */
	zassert_equal(osal_event_wait(e, EVT_A | EVT_B, true, false, OSAL_NO_WAIT), 0,
		      "wait_all returned with only a partial match");
	osal_event_delete(e);
}

/* -------------------------------------------------------------------------
 * NULL-handle guards: every primitive must treat a NULL handle as a safe,
 * non-crashing no-op (or the documented error code) rather than dereferencing.
 * -------------------------------------------------------------------------
 */
ZTEST(osal, test_null_guards)
{
	uint32_t out = 0;
	...
	/* Semaphore */
	zassert_equal(osal_sem_take(NULL, OSAL_NO_WAIT), OSAL_INVAL);
	zassert_equal(osal_sem_give(NULL), OSAL_INVAL);
	zassert_equal(osal_sem_count(NULL), 0);
	osal_sem_reset(NULL);
	osal_sem_delete(NULL);

	/* Queue */
	zassert_equal(osal_queue_send(NULL, &out, OSAL_NO_WAIT), OSAL_INVAL);
	zassert_equal(osal_queue_send_to_front(NULL, &out, OSAL_NO_WAIT), OSAL_INVAL);
	zassert_equal(osal_queue_send_from_isr(NULL, &out), OSAL_INVAL);
	zassert_equal(osal_queue_recv(NULL, &out, OSAL_NO_WAIT), OSAL_INVAL);
	zassert_equal(osal_queue_peek(NULL, &out, OSAL_NO_WAIT), OSAL_INVAL);
	zassert_equal(osal_queue_count(NULL), 0);
	osal_queue_delete(NULL);

	/* Event */
	zassert_equal(osal_event_set(NULL, EVT_A), 0);
	zassert_equal(osal_event_clear(NULL, EVT_A), 0);
	zassert_equal(osal_event_wait(NULL, EVT_A, false, false, OSAL_NO_WAIT), 0);
	osal_event_delete(NULL);

	/* FIFO */
	osal_fifo_put(NULL, &out);
	osal_fifo_put_front(NULL, &out);
	zassert_is_null(osal_fifo_get(NULL, OSAL_NO_WAIT));
	zassert_false(osal_fifo_remove(NULL, &out));
	osal_fifo_delete(NULL);

	/* Memory pool */
	zassert_is_null(osal_mempool_alloc(NULL, OSAL_NO_WAIT));
	osal_mempool_free(NULL, &out);
	zassert_equal(osal_mempool_used(NULL), 0);
	osal_mempool_delete(NULL);

	/* Timer */
	zassert_equal(osal_timer_start(NULL, 10), OSAL_INVAL);
	zassert_equal(osal_timer_stop(NULL), OSAL_INVAL);
	osal_timer_delete(NULL);

	/* Thread */
	osal_thread_suspend(NULL);
	osal_thread_resume(NULL);
	zassert_equal(osal_thread_priority_get(NULL), 0);
	osal_thread_priority_set(NULL, PRIO);
	osal_thread_delete(NULL);

	/* Locks */
	zassert_equal(osal_spin_lock(NULL), 0);
	osal_spin_unlock(NULL, 0);
	osal_spinlock_delete(NULL);
	osal_crit_enter(NULL);
	osal_crit_exit(NULL);
	osal_crit_delete(NULL);
}

/* -------------------------------------------------------------------------
 * Invalid create arguments: zero sizes/counts must fail cleanly with NULL,
 * and a semaphore whose initial count exceeds its limit must be rejected.
 * -------------------------------------------------------------------------
 */
ZTEST(osal, test_invalid_create_args)
{
	zassert_is_null(osal_queue_create(0, sizeof(uint32_t)));
	zassert_is_null(osal_queue_create(4, 0));
	zassert_is_null(osal_mempool_create(0, 32));
	zassert_is_null(osal_mempool_create(3, 0));
	/* init_count > max_count is rejected by k_sem_init. */
	zassert_is_null(osal_sem_create(1, 2));
}

/* -------------------------------------------------------------------------
 * Static-pool exhaustion: with CONFIG_OSAL_DYNAMIC_ALLOC=n the control-block
 * and thread pools are bounded. Creating past the bound must return NULL
 * gracefully rather than fault. Only meaningful in the static configuration.
 * -------------------------------------------------------------------------
 */
...
{
	ARG_UNUSED(arg);
	/* Park until aborted by the test cleanup. */
	while (true) {
		osal_delay_ms(1000);
	}
}

ZTEST(osal, test_static_thread_pool_exhaustion)
{
	osal_thread_t threads[CONFIG_OSAL_MAX_THREADS + 2];
	int created = 0;

	for (int i = 0; i < CONFIG_OSAL_MAX_THREADS + 2; i++) {
		threads[i] = osal_thread_create(exhaust_worker, "ex", NULL,
						CONFIG_OSAL_THREAD_STACK_SIZE, PRIO);
		if (threads[i] != NULL) {
			created++;
		}
	}

	zassert_equal(created, CONFIG_OSAL_MAX_THREADS,
		      "expected exactly %d threads, created %d",
		      CONFIG_OSAL_MAX_THREADS, created);

	for (int i = 0; i < CONFIG_OSAL_MAX_THREADS + 2; i++) {
		if (threads[i] != NULL) {
			osal_thread_delete(threads[i]);
		}
	}

	/* A slot must be reusable after the pool is drained again. */
	osal_thread_t t = osal_thread_create(exhaust_worker, "reuse", NULL,
					     CONFIG_OSAL_THREAD_STACK_SIZE, PRIO);
	zassert_not_null(t, "stack slot not reclaimed after delete");
	osal_thread_delete(t);
}

/* A stack request larger than the static pool slot must be rejected. */
ZTEST(osal, test_static_oversized_stack)
{
	osal_thread_t t = osal_thread_create(exhaust_worker, "big", NULL,
					     CONFIG_OSAL_THREAD_STACK_SIZE * 4, PRIO);

	zassert_is_null(t, "oversized stack request was not rejected");
}
