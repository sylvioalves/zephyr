...
disagree about what priority that line should run at, so
:kconfig:option:`CONFIG_INT_BENCH_ALT_LINE_USE` picks between them. The
direct comparison is the default where the architecture supports direct
interrupts, nesting where it does not, and ``prj.nested.conf`` and
``prj.zli.conf`` select the other two, because how an interrupt is connected is fixed at
